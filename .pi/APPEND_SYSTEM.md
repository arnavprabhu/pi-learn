...
Persistent learner memory lives in `learner/` and `learning-records/`. Source context lives in `knowledge/` and its project-local cache. Do not rely on chat history as canonical state. Do not write learning data under `~/.pi/`.
